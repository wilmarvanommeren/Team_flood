...
shinyServer(function(input, output){#Create plot from the inputvariables
  output$plot<-renderPlot({
    ...
    DEM.withbreach <- withProgress(expr=try(merge.breach.DEM(breach.area, DEM))
                                   , message = '(Re-)Calculation in progress',detail = 'Step 1: Merging breach with DEM...')
    
    # Calculate flooded area
    flooded.area<-NULL
    flooded.area<- withProgress(expr=try(isolate(calculate.flooded.area(breach.area, water.height, DEM, DEM.withbreach)))
                                , message = '(Re-)Calculation in progress',detail = 'Step 2: Calculating the flooded area...')
    ...
    if (input$goButton==0){
      ...
      withProgress(spplot(DEM, col.regions=DEMPallette, main='Digital Elevation Map', sub="Height [m] ",
                          xlab='Longitude',ylab='Latitude', 
                          scales = list(draw = TRUE)), message = '(Re-)Calculation in progress',detail = 'Step 3: Plotting the map...')} 
    else {
      withProgress(spplot (flooded.area, col.regions = WATERPallette, 
                           main='Flooded Area', sub='Waterheight [m]',
                           xlab='Longitude',ylab='Latitude', 
                           scales = list(draw = TRUE), 
                           sp.layout=list(list('sp.polygons', breach.area, 
                                               col='red', fill='red', first=FALSE)))+
                     as.layer(spplot(DEM, col.regions=DEMPallette), under=T), message = '(Re-)Calculation in progress',detail = 'Step 3: Plotting the map...')}        
  })
})